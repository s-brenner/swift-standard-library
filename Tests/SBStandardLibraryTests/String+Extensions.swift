...
final class StringExtensionsTests: XCTestCase {
    ...
    func testCamelCased() {
        XCTAssertEqual("SoMe vAriable naMe".formatted(.camelcased), "someVariableName")
        XCTAssertEqual("some_variable_name".formatted(.camelcased), "someVariableName")
    }
    
    func testBase64() {
        let string = "test:secret"
        let base64 = string.formatted(.base64Encoded())
        let random = String(randomWithLength: 100)
        XCTAssertEqual(base64, "dGVzdDpzZWNyZXQ=")
        XCTAssertEqual(base64.base64Decoded(), string)
        XCTAssertEqual(random.formatted(.base64Encoded()).base64Decoded(), random)
    }
